...
setup_models_dir() {
  ...
  # Grant llamafleet read+write+execute on the models dir itself (so it can
  # create downloaded files there), and read+execute on every parent directory
  # so it can traverse into mounted drives with restricted permissions.
  if command -v setfacl >/dev/null 2>&1; then
    setfacl -m u:llamafleet:rwx "$real_dir" 2>/dev/null || true
    local dir
    dir="$(dirname "$real_dir")"
    while [[ "$dir" != "/" ]]; do
      setfacl -m u:llamafleet:rx "$dir" 2>/dev/null || true
      dir="$(dirname "$dir")"
    done
    echo "ACL permissions granted for llamafleet on models path."
  else
    echo "Warning: setfacl not available (install acl package)."
    echo "  Grant read/write access manually: chmod o+rwx $real_dir"
    echo "  Grant traverse access on each parent up to $real_dir: chmod o+rx <dir>"
  fi
